...
/** Squad identities only; coordinates come from the selected formation. */
export const genericTacticalPlayers=Array.from({length:10},(_,i)=>({id:`${i<5?'H':'A'}${i%5+1}`,team:(i<5?'home':'away') as 'home'|'away',name:`Player ${i%5+1}`,x:0,y:0,teamRole:'',instruction:'',captain:false}));
