...
export async function saveCapture(data:string|Blob,name='TAAMEN capture'){const blob=typeof data==='string'?fromDataUrl(data):data;if(blob.size>8*1024*1024)throw new Error('capture-too-large');const item:LocalCapture={id:crypto.randomUUID(),blob,createdAt:Date.now(),name};await putItem(STORE,item);return item}
...
export function objectUrl(item:LocalCapture){return URL.createObjectURL(item.blob)}
